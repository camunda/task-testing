...
export default class TaskExecution extends EventEmitter {
  ...
  async executeTask(element, variables) {
    ...
    if (!deploymentResponse.success) {
      const error = { message: 'Failed to deploy process definition', response: deploymentResponse.error };
    ...
    if (!processDefinitionKey) {
      const error = { message: 'Failed to retrieve process definition key from deployment response', response: deploymentResponse.response };
    ...
    if (!startInstanceResponse.success) {
      const error = { message: 'Failed to start process instance', response: startInstanceResponse.error };
    ...
    const intervalCallback = async () => {
      ...
      if (!processInstanceResponse.success) {
        this._cancelTaskExecution(TASK_EXECUTION_FINISHED_REASON.ERROR, {
          ...
          response: processInstanceResponse.error
        });
      ...
      const responses = await Promise.all([
        ...
        this._api.getProcessInstanceJobs(processInstanceKey, element.id),
        this._api.getProcessInstanceMessageSubscriptions(processInstanceKey, element.id),
        this._api.getProcessInstanceUserTasks(processInstanceKey, element.id),
      ...
      if (errorResult) {
        this._cancelTaskExecution(TASK_EXECUTION_FINISHED_REASON.ERROR, /** @type {TaskExecutionError} */ {
          message: 'Failed to poll process instance data',
