...
/**
 * Number of polls to wait for the tested element's instance to reach a
 * terminal state in the search index after the process instance is already
 * reported terminal. The process and element instance indices are eventually
 * consistent and updated independently, so the element state can lag behind
 * briefly. Without this grace period, fast tasks (e.g. script tasks) can be
 * reported as terminated although they completed successfully.
 */
const ELEMENT_INSTANCE_SYNC_POLLS = 5;
...
export default class TaskExecution extends EventEmitter {
  ...
  constructor(injector, api) {
    ...
    this._elementInstanceSyncPollsRemaining = 0;
  ...
  async executeTask(element, variables) {
    ...
    this._lastPolledResult = null;
    this._elementInstanceSyncPollsRemaining = ELEMENT_INSTANCE_SYNC_POLLS;
    ...
    const intervalCallback = async () => {
      ...
      if (isCompleted || isTerminated) {
        ...
        const isElementTerminal = elementInstance && (
          elementInstance.state === ELEMENT_INSTANCE_STATE.COMPLETED
          || elementInstance.state === ELEMENT_INSTANCE_STATE.TERMINATED
        );

        // The element instance index may lag behind the process instance
        // index; keep polling while synchronization polls remain instead of
        // reporting termination based on a skewed snapshot.
        if (!isElementTerminal && this._elementInstanceSyncPollsRemaining > 0) {
          this._elementInstanceSyncPollsRemaining--;
