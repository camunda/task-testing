...
import { parseValueFromString, getTypeFromValue } from './utils/variables';

export const INTERVAL_MS = 1000;

/** @type {{ [key: string]: import('./types').VARIABLE_SCOPE }} */
export const SCOPES = {
  LOCAL: 'LOCAL',
  PROCESS: 'PROCESS'
};
...
export default class TaskExecution extends EventEmitter {
  ...
  async executeTask(element, variables) {
    ...
    const startInstanceResult = await this._api.startInstance(processId, element.id, variables);
 ...
 */
export function getVariables(getVariablesResponseItems, getElementInstancesResponseItems, processInstance, element) {
  const variables = {};

  for (const getVariablesResponseItem of getVariablesResponseItems) {
    const { name, value, variableKey } = getVariablesResponseItem;

    const scope = getScope(getVariablesResponseItem, getElementInstancesResponseItems, processInstance, element.id);
    const parsedValue = parseValueFromString(value);
    ...
    variables[variableKey] = {
      ...
      scope,
      source: 'OUTPUT',
      type: getTypeFromValue(parsedValue),
      origin: element
    };
  }

  // @ts-ignore
  // TODO: Can be fixed with a `reduce` instead of `for...of` loop
  return variables;
}
