 ...
 */
export default async function run(elementId, variables, api, log) {
  ...
  if (!pingResult) {
    log(elementId, 'Process instance not found after maximum attempts');
  } else {
    log(elementId, 'Execution result:');
    log(elementId, JSON.stringify(pingResult.response.variables, null, 2), 'result');
  }
}
