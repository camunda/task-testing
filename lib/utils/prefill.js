...
/**
 * Collect the distinct origin element names across the variables consumed by
 * the element, so the UI can show where the prefilled values come from.
 *
 * @param {Object} elementVariables
 * @param {Object} element
 * @returns {Promise<string[]>} distinct origin names
 */
export async function getPrefillSources(elementVariables, element) {
  const consumed = await elementVariables
    .getConsumedVariablesForElement(element);

  const names = [];

  for (const variable of consumed) {
    for (const { name } of variable.origin || []) {
