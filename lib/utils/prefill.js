...
/**
 * Returns true if the input string represents an empty config:
 * either `{}` or an object where all nested values are null.
 *
 * @param {string} inputString
 * @returns {boolean}
 */
export function isInputEmpty(inputString) {
  const str = isString(inputString) ? inputString : DEFAULT_INPUT_CONFIG;
  ...
  return Object.keys(removeNullValues(parsed)).length === 0;
}
