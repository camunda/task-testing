...
export default class ExecutionLog {
  ...
  _updateEntries() {
    ...
    // timestamps can arrive slightly out of causal order.
    entries.sort((a, b) => {
      if (isStatusEntry(a) || isStatusEntry(b)) {
        return compareEntryOrders(a, b);
      } else if (!areEntryTimestampsEqual(a, b)) {
        return compareEntryTimestamps(a, b);
      } else if (areEntriesRelated(a, b)) {
        return compareEntryOrders(a, b);
      }
