...
import { generatePrefillInput } from '../../utils/prefillUtils';
...
export default function TaskTesting({
  ...
  // Helper function to perform auto-prefill
  const performAutoPrefill = useCallback(() => {
    if (!element || !elementConfigRef.current) {
      return;
    }

    // Get current input for the element
    const currentInput = elementConfigRef.current.getInputConfigForElement(element) || '{}';

    // Generate prefill input based on FEEL expressions
    const prefillInput = generatePrefillInput(element, currentInput);

    // Only update if there's new prefill data that's different from current
    if (prefillInput && prefillInput !== currentInput) {
      elementConfigRef.current.setInputConfigForElement(element, prefillInput);
    }
  }, [ element, injector ]);

  // Auto-prefill input when element changes
  useEffect(() => {
    performAutoPrefill();
  }, [ element, performAutoPrefill ]);

  // Auto-prefill when element's expressions change (via commandStack)
  // Track input/output config changes to trigger prefill updates
  useEffect(() => {
    if (!element || !elementConfigRef.current) {
      return;
    }

    // Get initial snapshot to detect changes
    let lastSnapshot = JSON.stringify(element.businessObject);

    // Debounced prefill function
    let prefillTimeout;
    const debouncedPrefill = () => {
      clearTimeout(prefillTimeout);
      prefillTimeout = setTimeout(() => {
        const currentSnapshot = JSON.stringify(element.businessObject);

        // Only prefill if businessObject actually changed
      ...
      }, 300); // Debounce by 300ms to batch changes
    };

    // Check for property changes periodically
    const checkInterval = setInterval(() => {
      if (element.businessObject) {
        const currentSnapshot = JSON.stringify(element.businessObject);

        // Only trigger prefill if something changed
        if (currentSnapshot !== lastSnapshot) {
          debouncedPrefill();
        }
      }
    }, 500); // Check every 2 seconds (less expensive than 500ms)
