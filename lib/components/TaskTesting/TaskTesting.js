...
import { Chemistry, Cursor_1 as Cursor, Play, Stop, WarningFilled } from '@carbon/icons-react';
 ...
 * @param {import('react').MouseEventHandler} [props.onMissingDeployConfig=() => {}]
 */
export default function TaskTesting({
  ...
  const handleTaskExecution = async () => {
    if (!element || !!inputError || !taskExecutionRef.current || !elementConfigRef.current) {
      return;
    }
      ...
      <div className="task-testing__container--header">
        ...
        <div className="task-testing__container--header--right">
          <Controls
            ...
            onMissingDeployConfig={ onMissingDeployConfig }
          />
        </div>
      </div>
...
function Controls({
  ...
  const CANNOT_EXECUTE_TOOLTIP = 'Provide valid deployment configuration';
  const INVALID_JSON_TOOLTIP = 'Provide a valid JSON input';

  if (!canExecuteTask) {
    return (
      <Tooltip label={ CANNOT_EXECUTE_TOOLTIP } align="left-start">
        <Button
          className="btn-error"
          kind="tertiary"
          onClick={ onMissingDeployConfig }
  ...
  if (isInputError) {
    return (
      <Tooltip label={ INVALID_JSON_TOOLTIP } align="left-start">
        <Button
          kind="tertiary"
          onClick={ () => {} }
          renderIcon={ WarningFilled }
  ...
  if (isTaskExecuting) {
    return (
      <Button kind="tertiary" onClick={ onCancelTaskExecution } renderIcon={ Stop } size="sm">
        { CANCEL_LABEL }
      </Button>
    );
  }
  ...
  return (
    <Button kind="primary" onClick={ onExecuteTask } renderIcon={ Play } size="sm">
      { EXECUTE_LABEL }
    </Button>
  );
}