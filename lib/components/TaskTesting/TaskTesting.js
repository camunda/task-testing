...
export default function TaskTesting({
  ...
  useEffect(() => {
    ...
    elementConfigRef.current.on('config.changed', onConfigChanged);

    return () => {
      if (elementConfigRef.current) {
        elementConfigRef.current.off('config.changed', onConfigChanged);
      }
    };
  ...
  useEffect(() => {
    ...
    const onTaskExecutionError = ({ message, error }) => {
        ...
        <div className="task-testing__container--header--right">
          <Tooltip className={ (!canExecuteTask || inputError) ? 'has-error' : '' } label="Cannot test task" align="left-start">
            <Button
              ...
              size="sm"
              onClick={ isTaskExecuting ? onCancelTaskExecution : onExecuteTask }>
