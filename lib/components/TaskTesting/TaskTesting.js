...
/**
 * @param {Parameters<typeof TaskTesting>[0]} props
 */
function TaskTestingPanel({
  injector,
  api,
  hasError,
  ...
  if (!config) {
    return (
      <div className="task-testing__container task-testing__container--empty">
        <EmptyState size="sm" headingLevel={ 3 } heading="Configuring..." icon={ <Spinner /> } />
      </div>
    );
  }
    ...
    return (
      <div className="task-testing__container task-testing__container--empty">
        <EmptyState
          size="sm"
          headingLevel={ 3 }
          heading={ selectionInfo.title || selectionInfo.message }
          description={ selectionInfo.title ? selectionInfo.message : undefined }
          icon={ <Cursor aria-hidden="true" /> }
          action={ documentationUrl && <Link href={ documentationUrl } target="_blank">Learn more</Link> }
        />
      </div>
    );
  }
  ...
  const inputBadge = inputError
    ...
    : inputVariableCount > 0
      ? <Badge variant="neutral" className="task-testing-group__badge">{ inputVariableCount } { inputVariableCount === 1 ? 'variable' : 'variables' }</Badge>
      : null;
  ...
  const testTaskButton = (
    <Button
      data-testid="test-task-btn"
      variant={ showTooltip || isTaskExecuting ? 'secondary' : 'default' }
      size="sm"
      ...
      <div className="task-testing__container">
        <div className="task-testing__container--header">
          <div className="task-testing__header-actions">
            { showTooltip ? (
              <Tooltip>
                <TooltipTrigger asChild>{ testTaskButton }</TooltipTrigger>
                <TooltipContent side="bottom" align="start">{ tooltipLabel }</TooltipContent>
              </Tooltip>
            ) : testTaskButton }
          </div>
          <div className="task-testing__header-actions-right">
            {
              onConfigure && <IconButton
                data-testid="configure-btn"
                variant="ghost"
                size="sm"
                label={ configureTooltip }
                icon={ Settings }
                onClick={ () => onConfigure() }
