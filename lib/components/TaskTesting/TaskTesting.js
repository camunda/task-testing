 ...
 * @param {(element: Element, path: string, value: *) => void} [props.onAddToExampleData] - Called when the user saves a result value as example data for the element.
    ...
    <PluginContext.Provider value={ pluginsProviderValue }>
      <div className="task-testing__container">
        ...
        <div className={ `task-testing__container--body${isTaskExecuting ? ' task-testing__container--body-executing' : ''}` }>
          { (() => {
            const testPanel = (
              <div className="task-testing__test-panel">
                <CollapsibleSection
                  title="Input"
                  open={ isInputOpen }
                  onToggle={ setIsInputOpen }
            ...
            // panel directly. With plugin tabs, keep the Tabs strip.
            if (pluginTabs.length === 0) {
              return <div className="task-testing-tabs__panel">{ testPanel }</div>;
            }
