...
import { FillsContext, createFillsManager } from '../shared/SlotFill';
 ...
 * @param {import('../shared/SlotFill').FillProvider[]} [props.plugins=[]] - Array of SlotFill FillProviders
 */
export default function TaskTesting({
  ...
  const fillsManager = useMemo(() => createFillsManager(), []);

  useEffect(() => {
    plugins.forEach(plugin => {
      fillsManager.register(plugin);
    });

    return () => {
      plugins.forEach(plugin => {
        fillsManager.unregister(plugin);
      });
    };
  }, [ plugins, fillsManager ]);
  ...
  return (
    <FillsContext.Provider value={ fillsManager }>
