 ...
 * @param {Function} [props.onTaskExecutionUpdated=() => {}]
...
export default function TaskTesting({
  ...
  useEffect(() => {
    ...
    taskExecutionRef?.current?.on('taskExecution.update', handleUpdate);
    ...
    return () => {
      if (taskExecutionRef.current) {
        ...
        taskExecutionRef.current.off('taskExecution.update', handleUpdate);
  ...
  const handleExecuteTask = async () => {
    ...
    injector.get('toggleMode', false)?.toggleMode(true);
  ...
  const handleCancelTaskExecution = () => {
    ...
    injector.get('toggleMode', false)?.toggleMode(false);
  };
