...
import { getPrefillSources } from '../../utils/prefill';
 ...
 * @param {string} [props.connectionName] - Friendly name of the connected cluster, surfaced in the execution-is-real copy.
 ...
 * @param {(element: Element, sourceFeelExpression: string, targetName: string) => void} [props.onAppendOutputMapping] - Called when the user appends a result value to the element's output mapping.
 * @param {(element: Element, targetName: string) => void} [props.onNavigateToOutputMapping] - Called when the user requests navigation to an output mapping entry in the properties panel.
...
export default function TaskTesting({
  ...
  const [ prefillSources, setPrefillSources ] = useState(/** @type {string[]} */ ([]));
  ...
  // Controls the merged Test panel's Input section: open before a run,
  // collapsed on execute, reopenable by the user to tweak input and re-run.
  ...
  useEffect(() => {
    ...
    getPrefillSources(elementVariablesRef.current, element).then(sources => setPrefillSources(sources));
  }, [ element ]);
  ...
  useEffect(() => {
    ...
    const handleVariablesChanged = async () => {
      ...
      setPrefillSources(await getPrefillSources(elementVariablesRef.current, element));
  ...
  useEffect(() => {
    ...
    // A freshly selected element starts in the configure state.
    setIsInputOpen(true);
  }, [ element ]);
  ...
  const handleExecuteTask = async () => {
    ...
    // Collapse the Input section and switch to the Test tab so the Result
    // area takes over for the run (in case the user was on a plugin tab)
      ...
      <div className="task-testing__container">
        ...
        <div className={ `task-testing__container--body${isTaskExecuting ? ' task-testing__container--body-executing' : ''}` }>
          { (() => {
            ...
            const pluginTabs = pluginsProviderValue.getPlugins('output.body.tab')
              .map(plugin => ({
                label: plugin.label,
                disabled: isTaskExecuting,
            ...
            // Without plugin tabs there is no tab strip — render the merged Test
            // panel directly inside the scroll container. With plugin tabs, keep
            // the Tabs strip so the scroll container wrapping is provided by Tabs.
