...
export default function TaskTesting({
   ...
   * to open when no explicit value is present.
   *
   * @type {[Record<string, boolean>, React.Dispatch<React.SetStateAction<Record<string, boolean>>>]}
   */
  const [ openGroups, setOpenGroups ] = useState(/** @type {Record<string, boolean>} */ ({ input: true, result: true }));

  const toggleGroup = useCallback((key) => {
    setOpenGroups(groups => ({ ...groups, [key]: !(groups[key] ?? true) }));
  }, []);
  ...
  const handleExecuteTask = async () => {
    ...
    // Ensure the result group is expanded to show execution progress and results
    setOpenGroups(groups => ({ ...groups, result: true }));
    ...
    <C4Provider>
      <PluginContext.Provider value={ pluginsProviderValue }>
        <div className="task-testing__container">
          <div className={ `task-testing__container--header ${getHeaderStateClass({ isTaskExecuting, hasError, inputError, output })}` }>
...
function getHeaderStateClass({ isTaskExecuting, hasError, inputError, output }) {
  if (isTaskExecuting) {
    return 'task-testing__container--header-executing';
  } else if (output?.error || output?.incident || hasError || !!inputError) {
...
function HeaderStatusIcon({ isTaskExecuting, hasError, inputError, output }) {
  if (output?.error || output?.incident || hasError || inputError) {
...
function HeaderStatusText({ hasError, errorBannerTitle, inputError, output }) {
  if (hasError) {
    return errorBannerTitle;
  }
 ...
 * @param {React.MouseEventHandler<HTMLButtonElement>} [props.onToggle] - Called when the header is clicked.
 * @param {React.ReactNode} props.children - The group content.
 * @returns {React.ReactElement} The rendered collapsible group.
 */
function CollapsibleGroup({ label, open = true, onToggle = () => {}, children }) {
  return (
    <div className={ classNames('task-testing-group', { 'task-testing-group--open': open }) }>
      <button
        type="button"
      ...
      { open && (
        <div className="task-testing-group__content">
          { children }
        </div>
      ) }
    </div>
